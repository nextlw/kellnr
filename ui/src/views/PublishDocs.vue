<template>
  <div class="glass">
    ...
    <p class="k-p">
      ...
      If you just uploaded a new <i>crate</i> and no documentation is available, the <i>crate</i> is still in the queue and will be processed soon. For more information about the documentation features of <i>NexCode</i> see the official <a href="https://kellnr.io/documentation" class="link">documentation</a>.
    ...
    <p class="k-p">
      <i>NexCode</i> provides an easy option to host the corresponding
      ...
      need to be zipped and uploaded to <i>NexCode</i>.
    </p>
...
# Upload documentation to NexCode --> Replace values in brackets
curl -H "Authorization: {authorization token}" \
    http{s}://{NexCode host}/api/v1/docs/{crate name}/{crate version} \
    --upload-file {docs archive}</code></pre>

    <p class="k-p">
      To upload the documentation, <i>NexCode</i> checks that a <i>crate</i> with a
    ...
    <pre v-highlightjs class="glass"><code class="bash"># Example: Upload documentation to NexCode
curl -H "Authorization: Xjd83hDh45FLJ58bdShd4uhVNdnded4f" \
    http://nexcode.example.com:8000/api/v1/docs/mycrate/1.0.0 \
